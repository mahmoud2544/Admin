import { getRedis, cors, INTERACTION_LABELS, isoDate } from './_lib/redis.js';
import { requireSession } from './_lib/auth.js';

const LIVE_WINDOW_MS = 5 * 60 * 1000;
const WEEKS = 8;
const DAYS = WEEKS * 7; // 56 days of daily buckets, also covers the 14-day chart
...
// GET /api/analytics — admin only. Returns everything the dashboard renders.
export default async function handler(req, res) {
  ...
  for (let i = DAYS - 1; i >= 0; i--) {
    dates.push(isoDate(new Date(now - i * 86400000)));
  }
  ...
  return res.status(200).json({
    ...
    last14Days: {
      labels: last14Dates,
      sessions: last14.map((d) => d.s),
      pageViews: last14.map((d) => d.pv),
    },
    last8Weeks: weeks,
    devices: {
